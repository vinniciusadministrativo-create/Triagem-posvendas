...
export default function VendedorPage(){
  ...
  const submit=async()=>{
    ...
    const isTest = false; // Pode ser dinâmico no futuro

    try{
      // 1. Triagem
      setAgentStatus({triage:"running",doc:"waiting",evidence:"waiting"});
      const triageRes = await api.triageAI(form, isTest);
      ...
      if(triageRes.precisa_espelho_nfd && nfB64){
        ...
        docRes = await api.extractNFAI(nfB64, nfMime, isTest);
      ...
      if(evidenceFiles.length > 0){
        ...
        evRes = await api.analyzeEvidenceAI(imgs, isTest);
  ...
  return(
    <div style={{minHeight:"100vh",background:M.bg,padding:"40px 20px 40px 90px",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",marginBottom:30}}>
        <h1 style={{fontSize:24,fontWeight:800,color:M.tx}}>Nova Solicitação de Pós-Vendas</h1>
        <p style={{color:M.txM}}>Preencha os dados e anexe a NF para triagem automática.</p>
      </div>

      {/* TABS */}
      <div style={{maxWidth:900,margin:"0 auto 20px",display:"flex",gap:10}}>
        <button onClick={()=>setActiveTab("novo")} style={{padding:"10px 20px",borderRadius:10,border:"none",background:activeTab==="novo"?M.pri:"#fff",color:activeTab==="novo"?"#fff":M.txM,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 10px rgba(0,0,0,0.05)"}}>+ Novo Chamado</button>
        <button onClick={()=>setActiveTab("meus")} style={{padding:"10px 20px",borderRadius:10,border:"none",background:activeTab==="meus"?M.pri:"#fff",color:activeTab==="meus"?"#fff":M.txM,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 10px rgba(0,0,0,0.05)"}}>📋 Meus Chamados</button>
      </div>

      {activeTab === "meus" ? (
        <div style={{maxWidth:900,margin:"0 auto",background:"#fff",borderRadius:14,padding:20,border:`1px solid ${M.brdN}`}}>
          {meusChamados.length === 0 ? <p style={{textAlign:"center",padding:40}}>Nenhum chamado encontrado.</p> : (
            meusChamados.map(c => (
              <div key={c.id} style={{padding:15,borderBottom:`1px solid ${M.brdN}`,display:"flex",justifyContent:"space-between"}}>
                <div>
                  <div style={{fontWeight:700}}>{c.razao_social}</div>
                  <div style={{fontSize:12,color:M.txM}}>NF {c.nf_original}</div>
                </div>
        ...
        <div style={{maxWidth:900,margin:"0 auto",background:"#fff",padding:30,borderRadius:14,border:`1px solid ${M.brdN}`}}>
          {step === 0 && (
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
              ...
              <button onClick={submit} style={{gridColumn:"1/-1",padding:15,background:M.pri,color:"#fff",border:"none",borderRadius:10,fontWeight:800,cursor:"pointer"}}>⚡ Iniciar Triagem IA</button>
            </div>
          )}

          {step === 1 && (
            <div style={{textAlign:"center",padding:40}}>
              <h2>🤖 Agentes de IA Trabalhando...</h2>
              <p>{elapsed}s</p>
              <div style={{maxWidth:300,margin:"0 auto",textAlign:"left",marginTop:20}}>
                <div style={{marginBottom:10}}><AgentDot status={agentStatus.triage}/> Classificando Chamado</div>
          ...
          {step === 2 && triageResult && (
            <div>
              ...
              <div style={{marginBottom:20}}>
                ...
                <textarea 
                  value={ressalvaVendedor} 
                  onChange={e=>setRessalvaVendedor(e.target.value)}
                  placeholder="Ex: Cliente tem pressa, produto está na caixa original..."
                  style={{width:"100%",padding:12,borderRadius:10,border:`1px solid ${M.brdN}`,minHeight:80,fontFamily:"inherit",fontSize:13,boxSizing:"border-box"}}
                />
              </div>
